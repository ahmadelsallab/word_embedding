...
% corresponding index.
function [mCode, nErr] = TST_computeAutoEncoderCode(mBatchData, NM_strNetParams, nBitfieldLength, vChunkLength, vOffset, eFeaturesMode)
    ...
    nCodeSize = size(NM_strNetParams.cWeights{NM_strNetParams.nNumLayers}, 2);
    %mReconstructedData = zeros(nNumExamples, nCodeSize, nNumBatches)
    mCode = [];
...
%   err= err +  1/N*sum(sum( (data(:,1:end-1)-dataout).^2 )); 
        ...
        % Append the result batch data codes
        %mReconstructedData(:,:,ctrBatch) = mReconstructedBatchData;
        mCode = [mCode; mAugActivationData{NM_strNetParams.nNumLayers}];
