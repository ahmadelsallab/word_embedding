    ...
    otherwise
        ...
        for layer = 1 : N_layers
            if(layer == 1 & bWordEmbedding == 1)
                nSecondLayerSize = floor(l(layer+1) / nGram);
                NW_weights{layer} = reshape(VV((offset+1) : (offset+(l(layer)+1)*nSecondLayerSize)), l(layer)+1, nSecondLayerSize);
                offset = offset + (l(layer)+1)*nSecondLayerSize;
            else
...
while (layer >= 1)
    ...
    if(bWordEmbedding == 1)
        ...
        if(layer == 1)
            ...
            % BP_layerInputData(i) = idx = [10, 150, 390, 40, 34] --> word indices
            % BP_layerInputData--> Ncases x N, N = n-grams = 5 for example
            for i = 1 : size(BP_layerInputData, 1)
                offset = 1;
                % N-gram = size(XX, 2) --> BP_layerInputData(i) = idx = [10, 150, 390, 40, 34] --> word indices
                % N * embedding_size = size(We, 2) = size(NW_weights{layer}, 2)
                embedding_size = nWordEmbeddingSize;
                for j = 1 : nGram
                    ...
                    dw{layer}(BP_layerInputData(i,j), :) = dw{layer}(BP_layerInputData(i,j), :) + Ix{layer}(i, offset:offset + embedding_size - 1);  
                    offset = offset + embedding_size;
                end
            end
